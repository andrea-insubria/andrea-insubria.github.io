...
###############################################################
###                                                       #####
###       RAPPRESENTAZIONE DATI A LIVELLO REGIONI         #####
###                                                       #####
###############################################################
...
plyRegCasiAss= datiRegioni %>% group_by(denominazione_regione) %>% 
  plot_ly(x = ~data, y = ~totale_casi, type = 'scatter', mode = 'lines', colors = ~denominazione_regione, hovertemplate = paste('<b>Totale casi</b>: %{y}', '<br><b>Giorno</b>: %{x}<br>'), name = ~denominazione_regione) %>% 
...
axis_y <- list(showgrid = T, zeroline = F, nticks = 10, showline = T, title = 'Numero di decessi ogni 100000 abitanti', mirror = 'all', titlefont = f1) 

plyRegDecessi = datiRegioni %>% mutate(decessiRelativi = deceduti / popolazione * 100000) %>% group_by(denominazione_regione) %>% 
...
datiRegioniTi = as_tibble(datiRegioni)
datiRegioniTi <- datiRegioniTi %>% mutate(ospedalizzati = ricoverati_con_sintomi + terapia_intensiva)

axis_y <- list(showgrid = T, zeroline = F, nticks = 10, showline = T, title = 'Numero di ospedalizzati ogni 1000 abitanti', mirror = 'all', titlefont = f1) 

plyRegDecessi = datiRegioniTi %>% mutate(tassiOsp = ospedalizzati / popolazione * 1000) %>% 
  group_by(denominazione_regione) %>% 
  plot_ly(x = ~data, y = ~tassiOsp, colors = ~denominazione_regione, type = 'scatter', mode = 'line', hovertemplate = paste('<b>Tasso osp. </b>: %{y}', '<br><b>Giorno</b>: %{x}<br>'), name = ~denominazione_regione) %>% 
  layout(xaxis = axis_x, yaxis = axis_y, xaxis = axis_x, legend = list(x = .05, y = .95, title = 'Regioni')) 
plyRegDecessi


# ###############################################################
# ###                                                       #####
# ###          RAPPRESENTAZIONE DATI CON MAPPA              #####
# ###                                                       #####
# ###############################################################
# 
# ## plot dati a livello regionale: totale positivi ####

datiRegioniToday = as.data.frame(datiRegioniTi %>% filter(data == oggi))
datiRegioniToday = merge(italyShpRegions, datiRegioniToday, by.x = 'COD_REG', by.y = "codice_regione")


plyCasi <- plot_ly(datiRegioniToday, split = ~DEN_REG, color = ~totale_casi / popolazione * 1000, type = 'scatter', mode = 'lines', line = list(width = .1), text = ~paste('<b>', DEN_REG, '<b>','\n<b>Casi </b>:', round(totale_casi  / popolazione * 1000, 2)), hoveron = 'fills', hoverinfo = 'text') %>%
  layout(title = 'Proporzione positivi        Proporzione casi totali', legend = list(x = -0, y = .9,  title = list(text = 'Regione')), showlegend = FALSE) %>%
  colorbar(title = 'Casi ogni\n1000 ab.', x = .45, y = .85)

plyPositivi <- plot_ly(datiRegioniToday, split = ~DEN_REG, color = ~totale_positivi / popolazione * 1000, type = 'scatter', mode = 'lines', line = list(width = .1), text = ~paste('<b>', DEN_REG, '<b>','\n<b>Positivi </b>:', round(totale_positivi  / popolazione * 1000, 2) ), hoveron = 'fills', hoverinfo = 'text') %>%
  layout(legend = list(x = -0, y = .9,  title = list(text = 'Regione')), showlegend = FALSE) %>%
  colorbar(title = 'Positivi ogni\n1000 ab.', x = 1, y = .85)

plyOsp <- datiRegioniToday %>% 
  plot_ly(split = ~denominazione_regione, color = ~ospedalizzati / popolazione * 1000, type = 'scatter', mode = 'lines', line = list(width = .1), text = ~paste('<b>', denominazione_regione, '<b>','\n<b>Ospedalizzati </b>:', round(ospedalizzati  / popolazione * 1000, 2)), hoveron = 'fills', hoverinfo = 'text') %>%
  layout(legend = list(x = -0, y = .9,  title = list(text = 'Regione')), showlegend = FALSE) %>%
  colorbar(title = 'Osp. ogni\n1000 ab.', x = .45, y = .45)

plyDecessi <- datiRegioniToday %>% 
  plot_ly(split = ~denominazione_regione, color = ~deceduti / popolazione * 1000, type = 'scatter', mode = 'lines', line = list(width = .1), text = ~paste('<b>', denominazione_regione, '<b>','\n<b>Decessi </b>:', round(deceduti  / popolazione * 1000, 2)), hoveron = 'fills', hoverinfo = 'text') %>%
  layout(title = 'Proporzione ospedalizzati        Proporzione decessi', legend = list(x = -0, y = .9,  title = list(text = 'Regione')), showlegend = FALSE) %>%
  colorbar(title = 'Decessi ogni\n1000 ab.', x = 1, y = .45)


plyRaccolta = subplot(plyCasi, plyPositivi, plyOsp, plyDecessi, nrows = 2)
plyCasi



datiRegioni$nuovi_tamponi = c(datiNazionali$tamponi[1], diff(datiNazionali$tamponi))
...
## plotly graph regional data ####
## nuovi casi e variazione positivi
## setting axis
axis_x <- list(showgrid = T, zeroline = F, nticks = 20, showline = T, title = 'Data', mirror = 'all', tickangle = 45, titlefont = f1) 
axis_y <- list(showgrid = T, zeroline = F, nticks = 10, showline = T, title = 'Numero di casi', mirror = 'all', titlefont = f1) 

plyRegNuovi = plot_ly(data = datiRegioni, x = ~data, y = ~nuovi_positivi, color = ~denominazione_regione, type = 'scatter', mode = 'line', hovertemplate = paste('<b>Nuovi positivi</b>: %{y}', '<br><b>Giorno</b>: %{x}<br>'), name = ~denominazione_regione) %>% 
layout(xaxis = axis_x, yaxis = axis_y, xaxis = axis_x, legend = list(x = .05, y = .95)) 
