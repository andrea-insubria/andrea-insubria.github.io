...
load(paste0(prefixDir,'/shapefile_Rformat/all_shapefiles.Rdata'))

load(paste0(prefixDir,"/world_data/", tail(dir(paste0(prefixDir,"/world_data/")),1)))
...
data = dataOrig %>%  group_by(geoId) %>% arrange(countriesAndTerritories, dateRep) %>% mutate(cumCases = cumsum(cases_weekly), cumDeaths = cumsum(deaths_weekly))
...
plyCases = temp %>%
  filter(dateRep >= "2020-02-20") %>% group_by(geoId) %>%
  plot_ly(x = ~dateRep, y = ~cases_weekly, name = ~countriesAndTerritories, type = 'scatter', mode = 'lines', hovertemplate = paste('<b>Data: </b>%{x}\n', '<b>Casi: </b>%{y}')) %>%
  layout(xaxis = axis_x, yaxis = axis_y, title = 'Numero di casi', legend = list(x = .05, y = .95))
...
plyNewDeathsRate = temp %>%  mutate(newDeathsRate = deaths_weekly / popData2019 * 100000) %>%
...
tempVars = c("countryterritoryCode", "dateRep","cases_weekly","deaths_weekly", "popData2019","continentExp","cumCases","cumDeaths","CITY_NAME","CNTRY_NAME","X","Y") 
dataEuropeShp = dataEuropeShp[, tempVars]

## add rate of cases
dataEuropeShp = dataEuropeShp %>%
  mutate(cumCasesRate = cumCases/popData2019*1.e4, newCasesRate = cases_weekly /popData2019 * 1.e4, cumDeathsRate = cumDeaths/popData2019 *1.e5, newDeathsRate = deaths_weekly /popData2019 * 1.e5)
